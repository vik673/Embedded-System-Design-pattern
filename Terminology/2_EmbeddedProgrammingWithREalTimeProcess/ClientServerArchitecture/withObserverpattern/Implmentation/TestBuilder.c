...
static void initRelations(TestBuilder* const me) {
    /* Set up the relationships - ECG Module writes to the queue */
    ECG_Module_setItsTMDQueue(&(me->itsECG_Module), &(me->itsTMDQueue));
    
    /* All observers subscribe to the queue */
...
static void cleanUpRelations(TestBuilder* const me) {
    /* Relations will be cleaned up automatically by the individual components */
}
